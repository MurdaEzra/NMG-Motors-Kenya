...
import { cars, getSimilarCars, formatCurrency } from '../utils/carData';
...
const VehicleDetailPage = () => {
  ...
  // Get similar cars
  const similarCars = car ? getSimilarCars(car.id, car.brand) : [];
  ...
  useEffect(() => {
    ...
    // Set loaded state to false when car changes
    setIsLoaded(false);
    // Add animation effect with slight delay
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div className="flex p-2 overflow-x-auto">
              {car.images.map((image, index) => <div key={index} className={`h-16 w-24 flex-shrink-0 mx-1 cursor-pointer border-2 ${activeImageIndex === index ? 'border-[#A3320B]' : 'border-transparent'}`} onClick={() => setActiveImageIndex(index)}>
